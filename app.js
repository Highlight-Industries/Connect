...
function canonicalUrlForEmployee(id, view) {
  // Internal URL (where this app is hosted). Used for in-app navigation.
...
function getPublicBaseUrl() {
  // If provided (e.g., from Shopify embed), generate public links from this URL
  // so clients see Shopify instead of GitHub.
...
function publicProfileUrl(id) {
  // Responsive link (shows desktop on desktop, mobile on mobile)
...
function wireMobileControls() {
  ...
  // Share button (mobile): native share only. No QR modal on phones.
  els.shareBtnMob?.addEventListener("click", async () => {
    ...
    // Fallback (no Web Share API): show simple options (copy / text / email) — no QR.
  ...
  // Mobile share fallback buttons (no QR)
...
async function loadEmployees() {
  ...
  try {
    ...
    // Strip BOM if present on the first header cell
...
function registerServiceWorker() {
  // When embedded in Shopify (base=...), skip SW to avoid caching headaches.
