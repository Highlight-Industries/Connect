/* Highlight Digital Contact
   ...
   - Employee selected via #employee-id (preferred) or legacy ?u=employee-id
*/
...
const els = {
  ...
  shareModalMob: document.getElementById("shareModalMob"),
  copyLinkBtnMob: document.getElementById("copyLinkBtnMob"),
  textLinkBtnMob: document.getElementById("textLinkBtnMob"),
  emailLinkBtnMob: document.getElementById("emailLinkBtnMob"),
...
function renderEmployee(emp) {
  ...
  if (els.webDesk) {
    try {
      els.webDesk.textContent = emp.website ? new URL(emp.website).hostname : "—";
    } catch {
      els.webDesk.textContent = emp.website || "—";
    }
  }
  ...
  if (els.webMob) {
    try {
      els.webMob.textContent = emp.website ? new URL(emp.website).hostname : "—";
    } catch {
      els.webMob.textContent = emp.website || "—";
    }
  }
...
async function openVCard(emp) {
  const text = await vcardFor(emp);
...
async function vcardFor(emp) {
  ...
  try {
    const res = await fetch("./assets/building.jpg");
    ...
    console.warn("Could not load building image for vCard photo.", e);
  }
...
function wireDesktopControls() {
  ...
  if (els.saveBtnDesk) {
    els.saveBtnDesk.addEventListener("click", async () => {
      if (!current) return;
      const text = await vcardFor(current);
      downloadTextFile(`${current.id}.vcf`, text, "text/vcard;charset=utf-8");
    });
  }
  ...
  els.nativeShareBtn?.addEventListener("click", async () => {
    if (!current) return;
    const url = publicProfileUrl(current.id);
...
function wireMobileControls() {
  els.saveBtnMob?.addEventListener("click", async () => {
    if (!current) return;
    const text = await vcardFor(current);
    downloadTextFile(`${current.id}.vcf`, text, "text/vcard;charset=utf-8");
  });
  ...
  els.copyLinkBtnMob?.addEventListener("click", async () => {
    if (!current) return;
    await copyToClipboard(publicProfileUrl(current.id));
    toast("Link copied.");
  });

  els.textLinkBtnMob?.addEventListener("click", () => {
    if (!current) return;
    const url = publicProfileUrl(current.id);
    const msg = encodeURIComponent(`Here’s the contact for ${current.name}: ${url}`);
    location.href = `sms:?&body=${msg}`;
  ...
  els.emailLinkBtnMob?.addEventListener("click", () => {
    if (!current) return;
    const url = publicProfileUrl(current.id);
    const subject = encodeURIComponent(`Contact: ${current.name}`);
    const body = encodeURIComponent(`${current.name} — ${current.title}\n\n${url}`);
...
async function loadEmployees() {
  try {
    const csvUrl = cacheBust(absUrl("employees.csv"));
    const res = await fetch(csvUrl, { cache: "no-store" });
    console.log("[employees] CSV fetch:", res.status, res.url);
    ...
    console.warn("[employees] CSV load failed, trying JSON…", err);

    const jsonUrl = cacheBust(absUrl("employees.json"));
    const res = await fetch(jsonUrl, { cache: "no-store" });
    console.log("[employees] JSON fetch:", res.status, res.url);
